...
## Normal agents
class Agent:
    def __init__(self,location, color, palpha, ax, F, id, dimension):
        self.location = location.reshape(dimension,-1)
        self.locations = [[],[]]
        self.color = color
        self.palpha = palpha
    ...
    # f(x)
    def f(self):
        return np.array([0]*self.dimension).reshape(-1,1)
    # g(x)
    def g(self):
    ...
    def step(self, U, dt):
        ...
        def model(t, y):
            dydt = self.g()@ self.U
            return dydt.reshape(-1,self.dimension)[0]
        steps = solve_ivp(model, [0,dt], self.location.reshape(-1,self.dimension)[0])
        what = np.array([aa[-1] for aa in range(steps.y)])
    ...
    def agent_barrier(self,agent,d_min):
        ...
        dh_dxi = 2*( self.location - agent.location[:self.dimension]).T
        dh_dxj = -2*( self.location - agent.location[:self.dimension] ).T
